...
class TemporalConv(nn.Module):
    def __init__(self, input_size, hidden_size, conv_type=4, use_bn=False, num_classes=-1):
        ...
        self.temporal_model_combine = BiLSTMLayer(rnn_type='LSTM', input_size=hidden_size, hidden_size=hidden_size,
    ...
    def forward(self, frame_feat, lgt, use_layers=None):
        ...
        if use_layers == 4:
            ...
            lgt1=self.update_lgt1(lgt)
            visual_feat1=self.temporal_model_combine(visual_feat.transpose(1,2).transpose(0,1),lgt1.cpu())
            # visual_feat2=self.linear(visual_feat1['predictions'])
            visual_feat3 = self.K5_1(visual_feat1['predictions'].transpose(0,1).transpose(1,2))
            visual_feat4 = self.P2_1(visual_feat3)
            lgt2 = self.update_lgt1(lgt1)
            visual_feat=visual_feat4
        elif use_layers == 6:
            visual_feat = self.K5_0(visual_feat)
            visual_feat = self.P2_0(visual_feat)
...
class TemporalConvT(nn.Module):
    def __init__(self, input_size, hidden_size, conv_type=4, use_bn=False, num_classes=-1):
        super(TemporalConvT, self).__init__()
        self.use_bn = use_bn
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_classes = num_classes
...
class TemporalConvE(nn.Module):
    def __init__(self, input_size, hidden_size, conv_type=4, use_bn=False, num_classes=-1):
        super(TemporalConvE, self).__init__()
        self.use_bn = use_bn
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_classes = num_classes
