...
class TSEM(nn.Module):
    def __init__(self, input_size):
        ...
        self.conv_enhance = nn.ModuleList([
            nn.Conv1d(hidden_size, hidden_size, kernel_size=3, stride=1, padding=int(i + 1), groups=hidden_size,
                      dilation=int(i + 1)) for i in range(self.num)
        ])
...
class SSEM(nn.Module):
    def __init__(self, input_size):
        ...
        self.num = 3
        self.conv_enhance = nn.ModuleList([
            nn.Conv3d(div_channel, div_channel, kernel_size=(9, 3, 3), padding=(4, i + 1, i + 1),
                      dilation=(1, i + 1, i + 1), groups=div_channel) for i in range(self.num)
        ])
