...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, gloss_dict_T,gloss_dict_E,drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
        ...
        data_txt = f"DatasetFile/3004_new/{mode}_signer_third.txt"  # {mode}_3004_e.txt; {mode}_signer_continuous.txt; train_3004_e_third.txt;train_signer_continuous_third.txt; test_te_continuous_third.txt
    ...
    def __getitem__(self, idx):
        if self.data_type == "video":
            if self.mode == 'train':
                Tinput_data, Einput_data, label, Tlabel,Elabel,insert_word_before,insert_word_after,fi = self.read_video(idx)
                try:
                    Tinput_data, Einput_data, label = self.normalize(Tvideo=Tinput_data, Evideo=Einput_data, label=label)
                except:
                    print(fi)
                    exit()
                ...
                return Tinput_data, Einput_data, torch.LongTensor(label),torch.LongTensor(Tlabel), torch.LongTensor(Elabel),torch.LongTensor(insert_word_before),torch.LongTensor(insert_word_after),fi['insertFrameBefore'],fi['insertFrameAfter'],self.inputs_list[idx],'train'
            else:
    ...
    def read_video(self, index, num_glosses=-1):
        ...
        if self.mode == 'train':
            ...
            return [cv2.cvtColor(cv2.imread(i), cv2.COLOR_BGR2RGB) for i in Timg_list], \
                   [cv2.cvtColor(cv2.imread(j), cv2.COLOR_BGR2RGB) for j in Eimg_list], label_list, T_label_list,E_label_list,insert_word_before_list,insert_word_after_list,fi
        else:
            fi = self.inputs_list[index]
    ...
    def normalize(self, Tvideo=None, Evideo=None, Rvideo=None, label=None, file_id=None):
    ...
    def transform(self):
        if self.transform_mode == "train":
            ...
            return video_augmentation.Compose([
                ...
                video_augmentation.RandomCrop(112), ### 224 , 110, 180;112
                video_augmentation.RandomHorizontalFlip(0.5),
            ...
            return video_augmentation.Compose([
                # video_augmentation.ResizeShape((224, 224)),
                video_augmentation.CenterCrop(112), ###224 , 110, 180;112
                # video_augmentation.Resize(0.5),
                video_augmentation.ToTensor(),
            ])
    ...
    def collate_fn(batch):
        ...
        if batch[0][-1] == 'train':
            Tvideo, Evideo, label, Tlabel,Elabel,insertWordBefore,insertWordAfter,insertFrameBefore,insertFrameAfter,info,_ = list(zip(*batch))
            ...
            if max(label_length) == 0:
                ...
                return Tpadded_video, Epadded_video, Tvideo_length, Evideo_length, padded_label, label_length, padded_T_label,T_label_length,padded_E_label,E_label_length,padded_insertWordBefore,padded_insertWordAfter,insertFrameBeforeList,insertFrameAfterList,right_pad_list_T,right_pad_list_E,info
        else:
