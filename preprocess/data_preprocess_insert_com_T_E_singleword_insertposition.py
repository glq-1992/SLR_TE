        ...
        with open(save_path, "w") as f:
            for k, v in info.items():
                if not isinstance(k, int):
                    continue
                f.writelines(f"{v['Tfileid']} {v['Efileid']} 1 {v['signer']} 0.0 1.79769e+308 {v['label']}\n")
    else:
        with open(save_path, "w") as f:
            for k, v in info.items():
                # if not isinstance(k, int):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='te_all_third_new',
                        help='save prefix')
    parser.add_argument('--dataset_root', type=str, default='/disk2/dataset_glq/CSL_image_112',
    ...
    for md in mode:
        ...
        data_txt = f"../DatasetFile/3004_new/{md}_all_te_third.txt"
        # data_txt = f"../DatasetFile/transformer/csl_our/14000+CSL_{md}.txt"
        ...
        with open(data_txt) as f:
            if md == 'train':
                for i, line in enumerate(f.readlines()):
                    ...
                    Tfolder = Tfolder + '/*.jpg'
                    Efolder = Efolder + '/*.jpg'
                    information[i] = dict(Tfileid=Tfileid, Efileid=Efileid, signer=signer, Tfolder=Tfolder, Efolder=Efolder, label=label, Tlabel=Tlabel,Elabel=Elabel,insertWordBefore=insertWordBefore,insertWordAfter=insertWordAfter,insertFrameBefore=insertFrameBefore,insertFrameAfter=insertFrameAfter,original_info=line)
            else:
                for i, line in enumerate(f.readlines()):
                    # line = line.replace('\n', '')
                    # Tfolder, Efolder, label, Tlabel, Elabel, insertWordBefore, insertWordAfter, insertFrameBefore, insertFrameAfter = line.split(
                    #     'Q')
                    # Tfileid = Tfolder[-13:]
                    # Efileid = Efolder[-13:]
                    # signer = Tfolder[-7:-3]
                    # Tfolder = Tfolder + '/*.jpg'
                    # Efolder = Efolder + '/*.jpg'
                    # information[i] = dict(Tfileid=Tfileid, Efileid=Efileid, signer=signer, Tfolder=Tfolder,
                    #                       Efolder=Efolder, label=label, Tlabel=Tlabel, Elabel=Elabel,
                    #                       insertWordBefore=insertWordBefore, insertWordAfter=insertWordAfter,
                    #                       insertFrameBefore=insertFrameBefore, insertFrameAfter=insertFrameAfter,
                    #                       original_info=line)
                    ...
                    signer = folder[-7:-3]
                    folder = folder + '/*.jpg'
                    information[i] = dict(fileid=fileid, signer=signer, folder=folder, label=label, original_info=line)
